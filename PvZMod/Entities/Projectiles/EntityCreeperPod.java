...
public class EntityCreeperPod extends EntityPodBase
{
  ...
  public void damageType(MovingObjectPosition movObjPos)
  {
    ...
    if ((ent instanceof EntityCreeper)) {
      ...
      this.damage = (byte)(int)(damageMult * ((EntityCreeper)ent).getMaxHealth());
    }
...
int x = (int)this.g.posX;
      ...
      for (ItemStack itemStack : ghastDrops(0)); } else if ((ent instanceof EntityLivingBase)) {
      this.toDamage = false;
      hitSomething = true;
    }
  ...
  public void impactExtra()
  {
    if (this.g != null)
    {
      if (!(this.g instanceof EntityCreeperRepeater)) {
        float explosionRadius = (float)PvZ_Main.creeperPodRad;
  ...
  private ItemStack[] ghastDrops(int par2) {
    ...
    int tear = 0;
    for (int k = 0; k < j; k++) tear++;

    int powder = 0;
    j = this.rand.nextInt(3) + this.rand.nextInt(1 + par2);
    for (int k = 0; k < j; k++) powder++;

    ItemStack[] ghastDrops = { new ItemStack(Item.ghastTear, tear), new ItemStack(Item.gunpowder, powder + 1) };
