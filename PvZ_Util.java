...
public class PvZ_Util {
	...
	public static boolean checkUnder(World world, double posX, double posY,
		...
		for (double i = posX - par1; i <= posX + par1; i += 1.0D) {
			for (double k = posZ - par1; k <= posZ + par1; k += 1.0D) {
				if (!world.isBlockSolidOnSide((int) i, (int) posY, (int) k,
						ForgeDirection.UP)) {
					return false;
				}
			}
	...
	private static void spawnZombie(EntityZombie zombie, World world, double x,
		...
		for (int j = 0; j < 2; j++) {
			...
			if ((blockid == PvZ_Main.gravestone.blockID)
					|| (blockid == PvZ_Main.gravestoneReg.blockID)
					|| (blockid == PvZ_Main.gravestoneFootball.blockID)
					|| (blockid == PvZ_Main.gravestoneFlag.blockID)
					|| (blockid == PvZ_Main.gravestoneCone.blockID)
					|| (blockid == PvZ_Main.gravestoneBucket.blockID)) {
				continue;
			}

			if (blockid != 0) {
				blockIDs.add(Integer.valueOf(world.getBlockId(x1, (int) y + j,
						z1)));
		...
		if ((world.getBlockId(x1, (int) (y - 1.0D), z1) == 0)
				|| (!world.isBlockSolidOnSide(x1, (int) (y - 1.0D), z1,
						ForgeDirection.UP))) {
		...
		if (blockIDs.size() > 0)
			for (int i = 0; i < blockIDs.size(); i++) {
				if (((Integer) blockIDs.get(i)).intValue() == Block.grass.blockID)
					blockIDs.set(i, Integer.valueOf(Block.dirt.blockID));
				if (((Integer) blockIDs.get(i)).intValue() == Block.mycelium.blockID)
					blockIDs.set(i, Integer.valueOf(Block.dirt.blockID));
				if (((Integer) blockIDs.get(i)).intValue() == Block.stone.blockID)
					blockIDs.set(i, Integer.valueOf(Block.cobblestone.blockID));
				spawnItem(world, (int) x, (int) (y + 1.0D), (int) z,
						new ItemStack(((Integer) blockIDs.get(i)).intValue(),
								1, ((Integer) blockMeta.get(i)).intValue()));
			}
	}
	...
	public static boolean isItemInput(ItemStack itemStack) {
		return (itemStack.itemID == PvZ_Main.basicPlants.itemID)
				|| (itemStack.itemID == PvZ_Main.nightPlants.itemID);
	}

	public static boolean isItemModify1(ItemStack itemStack) {
		return (itemStack.itemID == PvZ_Main.sunlight.itemID)
				|| (itemStack.itemID == PvZ_Main.moonlight.itemID);
	}

	public static boolean isItemModify2(ItemStack itemStack) {
		return (itemStack.itemID == Item.snowball.itemID)
				|| (itemStack.itemID == Item.gunpowder.itemID);
	}
	...
	public boolean isSameEntity(Entity ent1, Entity ent2) {
		double ent1X = ent1.posX, ent1Y = ent1.posY, ent1Z = ent1.posZ;
		double ent2X = ent2.posX, ent2Y = ent2.posY, ent2Z = ent2.posZ;
		NBTTagCompound ent1Data = ent1.getEntityData(), ent2Data = ent2.getEntityData();
		String ent1Name = ent1.getEntityName(), ent2Name = ent2.getEntityName();
		if (ent1Name.equals(ent2Name)) {
			if (ent1Data.equals(ent2Data)) {
				if (ent1X == ent2X && ent1Y == ent2Y && ent1Z == ent2Z) {
					return true;
				}
			}
		}
